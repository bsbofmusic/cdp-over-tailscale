...
> ⚠️ 仓库已从`cdp-over-tailscale`更名为`cdp-bridge`，功能不变。
...
```mermaid
flowchart LR
    A[远端Agent/Tool] -->|Tailscale私网| B[CDP Bridge(Windows托盘)]
    B --> C[本地Chrome/Edge/Chromium]
...
> 💡 高级模式副本默认保存在Chrome用户目录附近的`CDP Bridge Profiles/`，自动识别复用。
...
### 🧹 清洁重装
如果应用行为异常、旧版本残留、托盘劫持，使用安装器中的「Clean reinstall」，自动清理旧进程、旧数据、旧快捷方式后重新安装。

---

## English
### What it does
AI agents need a real browser to bypass anti-scraping and access authenticated content. `cdp-bridge` is a Windows tray app that wraps your local Chrome in a token-authenticated CDP bridge, exposed **only over your private Tailscale network**. No port forwarding, no public exposure, zero firewall changes.

### How it works
```mermaid
flowchart LR
    A[Remote Agent (anywhere on Tailscale)] -->|Private network| B[CDP Bridge (Windows tray)]
...
2. Launch the app (it runs in the system tray)
...
| 浏览器启动失败 | 1. 确认本地Chrome/Edge安装到默认路径<br>2. 高级模式下检查profile是否有权限访问<br>3. 尝试清洁重装 |
...
npm start         # 本地运行
npm run check     # 语法检查
npm run dist:win  # 打包Windows安装器 → dist/CDP Bridge-Setup-x.x.x.exe
```
